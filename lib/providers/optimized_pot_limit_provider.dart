...
class OptimizedPotLimitProvider extends ChangeNotifier {
  // === 핵심 게임 상태 ===
  ...
  void _setupInitialBets() {
    ...
    if (sbIdx != -1 && _players[sbIdx].chips >= _smallBlind) {
      ...
      _playerActionHistory[sbIdx].add('SB: ${_formatAmount(_smallBlind)}');
    }
    ...
    if (bbIdx != -1 && _players[bbIdx].chips >= _bigBlind) {
      ...
      _playerActionHistory[bbIdx].add('BB: ${_formatAmount(_bigBlind)}');
    }
  }
  ...
  bool _performRaiseAction(Player player, int playerIndex, Random random) {
    ...
    if (maxRaise < minRaise) {
      ...
      _playerActionHistory[playerIndex].add('ALL-IN: ${_formatAmount(player.bet)}');
      return true;
    }
    ...
    final formattedAmount = _formatAmount(raiseAmount);
    final displayRaiseAmount = int.parse(formattedAmount);
  ...
  bool _performCallAction(Player player, int playerIndex) {
    ...
    if (playerCallDifference > player.chips) {
      ...
      if (player.position == Position.bigBlind || player.position == Position.smallBlind) {
        ...
        _playerActionHistory[playerIndex].add('CALL: ${_formatAmount(player.bet)}');
      } else {
        _playerActionHistory[playerIndex].add('CALL: ${_formatAmount(callAmount)}');
      }
    }
    return true;
  }
  ...
  Map<String, int> _calculatePotAction(Player player) {
    ...
    final formattedCallAmount = int.tryParse(_formatAmount(callAmount)) ?? callAmount;
    final formattedCurrentPot = int.tryParse(_formatAmount(currentPot)) ?? currentPot;
  ...
  void checkPotGuess() {
    ...
    if (isCorrect) {
      ...
      _resultMessage = '${AppLanguage.getText('wrongAnswer')}${AppLanguage.getText('correctAnswerIs')} ${_formatAmount(correctPot)}';
      _playSound('wrong');
    }
  ...
  int _calculateCurrentPot() {
    ...
    return int.tryParse(_formatAmount(rawCurrentPot)) ?? rawCurrentPot;
  }

  /// 콜 금액 계산
  int _calculateCallAmount() {
    final maxBet = _players.fold(0, (max, p) => p.bet > max ? p.bet : max);
    return int.tryParse(_formatAmount(maxBet)) ?? maxBet;
  }
...
''';
  ...
  String _buildPotDetailsString(List<Player> bettingPlayers, int currentPot) {
    ...
    final potDetailsStr = bettingPlayers.map((p) {
      ...
      final betStr = _formatAmount(p.bet);
